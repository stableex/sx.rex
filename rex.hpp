...
namespace rex {
     ...
     * - `{uint64_t} [ratio=10000]` - default ratio used to initialize supply
     *
     * ### example
     *
     * ```c++
     ...
     * const uint64_t supply = 1000000;
     * const uint64_t ratio = 10000;
     *
     * // Calculation
     * const uint64_t amount_out = rex::issue( payment, supply, deposit, ratio );
     * // => 100110
     * ```
     */
    static uint64_t issue( const uint64_t payment, const uint64_t deposit, const uint64_t supply, const uint64_t ratio = 10000 )
    {
        ...
        const uint64_t S0 = deposit; // deposit
        const uint64_t S1 = S0 + payment; // payment
        const uint64_t R0 = supply; // supply
        const uint64_t R1 = (uint128_t(S1) * R0) / S0;
